        ...
        if (feature.properties && feature.properties.popupContent) {
            layer
                ...
                .on("popupopen", () => {
                    ...
                    address.appendChild(
                        document.createTextNode(`${Town}, ${Address}`)
                    );
...
<div class="buttons">
    ...
    request.onupgradeneeded = () => {
        ...
        db.createObjectStore("connectionTypesList", { autoIncrement: true });
        db.createObjectStore("levelsList", { autoIncrement: true });
        db.createObjectStore("currentTypesList", { autoIncrement: true });
    };

    const idbSaveData = (storeName, station) => {
    ...
    const init = async () => {
      ...
      Title
        ...
        if (navigator.onLine) {
            ...
            try {
                idbClearData("stationList");
                stations.map(
                    async (station) => await idbSaveData("stationList", station)
                );
            } catch (error) {
                console.log("Error caching stations", error);
            }
        } else {
            console.log("is offline");
            stations = await idbLoadData("stationList");
    ...
    const fetchConnectionTypes = async (element) => {
      ...
      connectiontypes {
        ...
        if (navigator.onLine) {
            ...
            try {
                idbClearData("connectionTypesList");
                connectiontypes.map(
                    async (ct) => await idbSaveData("connectionTypesList", ct)
                );
            } catch (error) {
                console.log("Error caching connection types", error);
            }
        } else {
            connectiontypes = await idbLoadData("connectionTypesList");
        }
    ...
    const fetchLevels = async (element) => {
      ...
      leveltypes {
        ...
        if (navigator.onLine) {
            ...
            try {
                idbClearData("levelsList");
                leveltypes.map(
                    async (level) => await idbSaveData("levelsList", level)
                );
            } catch (error) {
                console.log("Error caching levels", error);
            }
        } else {
            leveltypes = await idbLoadData("levelsList");
        }
  ...
  `;
    ...
    currenttypes {
      ...
      Description
        ...
        if (navigator.onLine) {
            const data = await fetchGraphql(query);
            currenttypes = data.currenttypes;
            try {
                idbClearData("currentTypesList");
                currenttypes.map(
                    async (ct) => await idbSaveData("currentTypesList", ct)
                );
            } catch (error) {
                console.log("Error caching current types", error);
            }
        } else {
            currenttypes = await idbLoadData("currentTypesList");
        }
    ...
    const login = async (evt) => {
        ...
        for (let i = 0; i < loginForm.elements.length; i++) {
            if (loginForm.elements[i].tagName === "INPUT")
                values[loginForm.elements[i].name] =
                    loginForm.elements[i].value;
        }
        const query = {
            query: `{
...
`,
    ...
    const modifyConnectionBtn = document.querySelector(
        "#modify-connection-btn"
    );
  ...
  `;
    ...
    const modifyConnection = (id) => {
        ...
        for (const option of modifyForm.elements["modify-current-type"]
            .options) {
    ...
    const saveModConnection = () => {
        ...
        newConn.CurrentType.id =
            modifyForm.elements["modify-current-type"].value;
